...
class AppStore extends Component {
  state = {selectTabId: tabsList[0].tabId, inputValue: ''}

  selectedTab = tabId => {
    const resultTab = tabsList.filter(item => item.tabId === tabId)
    this.setState({selectTabId: resultTab[0].tabId})
  }
  ...
  render() {
    const {selectTabId, inputValue} = this.state

    const filteredAppsOnTab = appsList.filter(
      item => selectTabId === item.category,
    )

    const filteredApps = filteredAppsOnTab.filter(item =>
      item.appName.toLowerCase().includes(inputValue.toLowerCase()),
    )
        ...
        <ul className="list-container">
          {tabsList.map(item => (
            <TabItem
              ...
              isSelected={selectTabId === item.tabId}
              key={item.tabId}
            />
          ))}
        </ul>
        <ul className="list-container-app">
          {filteredApps.map(item => (
            <AppItem appItem={item} key={item.appId} />
          ))}
        </ul>
      </div>
    )
